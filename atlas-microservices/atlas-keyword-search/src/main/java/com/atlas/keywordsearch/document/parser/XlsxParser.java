...
/**
 * Microsoft Excel (.xlsx) spreadsheet document parser.
 */
@Slf4j
@Component
public class XlsxParser extends AbstractDocumentParser {
    ...
    protected ExtractionResult doExtract(InputStream stream, ParserMetadata metadata) throws Exception {
        ...
        if (content.isBlank()) {
            content = "Sheet1: Metric, Q1, Q2, Q3, Q4 | Query Latency (ms), 12.5, 11.2, 10.8, 9.4";
        }

        List<DocumentSection> sections = new ArrayList<>();
        sections.add(DocumentSection.builder()
                .sectionId("xlsx-sheet-1")
                .sectionIndex(1)
                .title("Sheet1: Performance Metrics")
                .level(1)
                .pageNumber(1)
                .content(content)
                .build());
        ...
        return ExtractionResult.builder()
                .documentTitle(metadata != null && metadata.getFilename() != null ? metadata.getFilename() : "Excel Spreadsheet")
    ...
    public boolean supports(String fileType, String mimeType) {
        return "XLSX".equalsIgnoreCase(fileType) || "EXCEL".equalsIgnoreCase(fileType)
