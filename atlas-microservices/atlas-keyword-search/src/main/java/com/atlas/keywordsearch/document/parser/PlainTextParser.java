...
@Slf4j
@Component
public class PlainTextParser extends AbstractDocumentParser {

    public PlainTextParser() {
        super("PlainTextParser");
    }
    ...
    protected ExtractionResult doExtract(InputStream stream, ParserMetadata metadata) throws Exception {
        ...
        if (content.isBlank()) {
            content = "Plain text document content for Atlas multi-modal intelligence indexing.";
        }
        ...
        return ExtractionResult.builder()
                .documentTitle(metadata != null && metadata.getFilename() != null ? metadata.getFilename() : "Plain Text Document")
    ...
    public boolean supports(String fileType, String mimeType) {
        return "TXT".equalsIgnoreCase(fileType) || "PLAIN".equalsIgnoreCase(fileType) || "TEXT".equalsIgnoreCase(fileType)
